...
-  further definition of our approach to tie formation and node attributes from Kiwulan burials
...
age_sex_by_phase <-
  ...
  pivot_wider(names_from = c(Phase, gender), 
              values_from = n) %>% 
  mutate(Age_scale = factor(Age_scale,
                            levels = c("0-12", "12-20", "+20", "NA"))) %>% 
  arrange(Age_scale) %>% 
...
ft_age_sex <- flextable(age_sex_by_phase) %>% 
  ...
  add_header_row(values = c("","male", "female", "NA", 
                            "male", "female", "NA"), top = FALSE) %>% 
  align(j = 2:7, align = "right", part = "header") %>% 
  align(i = 1, align = "center", part = "header") %>% 
  set_caption("The number of burials with identification of age and sex at Kiwulan. The NA means no sufficient skeletal remains for determination of age or sex")
ft_age_sex
```
