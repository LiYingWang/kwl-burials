...
Burials analysis has long developed in archaeology as an approach to study past social structure through mortuary practices, such as cultural and biological records in burial contexts, that can represent the social status or identity of the deceased [@Binford1971; @Saxe1970]. Despite the criticism for overlooking the burial manipulation by the living that does not necessarily connect to the real life [@Hodder1980; @Pearson1982], with comparisons between different archaeological contexts of death and life in mind, burials can still provide valuable information to infer past societies [@Chapman2003]. For example, mortuary practices, including burial forms, grave goods, and ritual behaviors, that structure the material configuration of burials can represent social relations between members of a community, allowing the inference for social structures [@Byrd1995; @Seikel2011]. To study the relations among burials, the recent development of network science provides many new tools to visualize and analyze relationships. Network science is the study of relational data where a phenomenon is conceptualized as a network through two steps, abstraction and representation, based on our specification in terms of network theories [@Brandes2013]. That is, a network can be viewed as a patterned aggregation that includes individual elements, pair-wise relationships (the dyads), and overall structure showing global patterning represented in data. 

A network is generally visualized as a graph consisting of a set of socially-relevant nodes, or actors, connected by edges representing one or more relations, such as friendship, collaborations, information flow, trade ties, or any other forms of connection of interest [@Marin2011; @Wasserman1994]. In archaeology, actors can be people, groups, objects, places, or events, with ties built on similarity, proximity, and co-presence of material culture based on theories for relationships [@Mills2017; @Brughmans2018]. For example, past trade can be conceptualized as a network of individual entities connected by shared similarity, the flow of goods, to represent their interactions [@Collar2015, pp. 4]. Similar concepts can be applied to a wide range of archaeological data with relational assumptions, such as burial analysis. Burial goods, especially for high value goods, can reflect social practices in broader cultural contexts to represent personal wealth or social status that can infer social differentiation or complexity [@Gamble2002; @Janes2013]. Burials with the same prestige goods could indicate some underlying social relations where individuals share similar access to trade, exchange, and gifting networks according to their status [eg. @Coward2013, pp. 252]. This enables the exploration of the structure of the past social organization by identifying the relationships among burials. 

Network analysis has been increasingly applied by archaeologists in recent years to deal with past interactions and explore the underlying mechanism. There are two common methodological approaches, one is directly examining network properties, such as node position and overall structures of networks built on archaeological data, to find meaningful patterns [@Mills2017]; another one is modeling theory to represent and test dependence of relationships [@Brughmans2018]. Node position is important to identify the role of a node in a network, such as its centrality, representing the individual influence or social prominence in a group, while network structure is helpful to generalize relationship patterns. Recent applications include long term inter-site relationship from the Epipalaeolithic to the early Neolithic in Near East according to trade items [@Coward2013], and hierarchical communication systems in the Kofun period in Japan through prestige goods [@Mizoguchi2013]. For modeling approaches, they provide a robust way to statistically test hypotheses about the relationships between actors to examine the structure of the network and major factors that affect the formation of the structure [@Freeman2004; @Salvini2010]. Examples include testing hunter-gatherer exchange network across the Kuril Islands based on ceramic compositions [@Erik2015], and modeling and comparing chronological and social hypotheses to explain the differences between burials [@Sosna2012]. This paper will use modeling approaches for the relationship between burials in northeastern Taiwan to explore the indirect impact of foreign contacts.
...
<!--chronology-->
To compare burial networks, we assigned burials to the pre-European period (n = `r pre_E`), European and post-European period (n = `r post_E`) for network comparison based on established fine-grained chronology that was reexamined and cross-validated by diagnostic materials, stratigraphic data, depth, and radiocarbon dates [@LWandBM2020ornament; @Chen2007]. We excluded the burials from the Chinese phase (n = `r chi`) for analysis due to the smaller sample size. There are eight burials heavily disturbed by modern construction that are excluded to avoid misinterpreting due to distorted chronology. We reconstructed social relations between individuals by linking them according to similar prestige goods. This is based on the assumption that the prestige goods imply social practices of trade, exchange, and gifting that reflect social relations between individuals [@Coward2013, pp. 252]. Also, prestige materials usually relate to a representation of social status because of their limited accessibility that can be used as status indicators [citations]. Therefore, we build networks where burials represent actors (nodes in the network) that are linked when they have the same prestige goods in common. The prestige goods we identified include agate beads, gold-foil beads, imported Chinese porcelains, gold foils, and fish-shaped ornaments that are considered as high-value items by studies across different archaeological contexts [@Cheng2008; @Hsieh2009; @Hsieh2012; @Wang2011] and the description in historical records [@Borao2009; @LiandWu2006]. 
