...
Burials contains valuable information about individual mortuary practices and inherited structure of a society as a whole. With the development of network science and graph theory, it is promising to study relation among burials and explore its social implication [@Sosna2013]. Mortuary practices such as burial forms, grave goods, and related ritual practices could represent the social relations between individuals and allow archaeologists to infer social structures. This enables the study of social differences based on burials to answer questions related to social inequality and complexity [@Ehrhardt2009; @Gamble2002; @Janes2013; @Johansen2015]. For examples, the use of burial goods can reflect social practices in a broader of cultural contexts to present personal wealth, social roles, or a traditional custom. Burials with grave goods in common often indicates underlying social relation connected through trade, exchange, and gifting [@Coward2013, pp. 252]. Identifying the structure of relationships among burials allows a further inferences on a structure of a social organization. A common way to study connections between individuals is network analysis, which has been applied increasingly to archaeological studies recently [@Mills2017]. Networks are represented as graphs consisting of a set of actors, such as people, groups, or organizations, and a set of links connecting them [@Brughmans2018; @Robins2007]. Technically speaking, actors are "nodes" connected by multiple "edges", representing relationships, between nodes. The basic unit of relationship is called dyad, defined as a connection between two distinct actors [@Lee2016]. In archaeology, relationships can be viewed as flows of resources that reflect relations of control, dependence, and cooperation, reflecting social relations in societies [@Marin2011]. With network analysis, we can quantify, visualize, and statistically test hypotheses of the relationships between actors to examine the structure of network and major factors that affects the structure [@Freeman2004; @Salvini2010]. The application in archaeology is widely used for [@Coward2013; @Mizoguchi2013; @Potgieter2008]

<!--archaeology case-->
The archaeological sites in northeastern Taiwan show evidence of use of imported prestige goods in burials and accumulation of them in residential area around the same time as European contact in the 17th century [@Chen2007], which might indicate the pursuit of prestige or wealth in the society. Those prestige goods include imported ornaments and porcelains that entered northeastern Taiwan through regional exchange networks involved the Europeans, Han Chinese traders, and Indigenous groups. In addition to prestige, European colonial powers could have provided military support to Indigenous allies. For example, @Voss2005's study of a colonial outpost in California shows that colonial residents have different identities and higher social status than other local Indigenous people. A general model to summarize these case studies might be that the support of a colonial power combined with high local values for imported goods, might lead to competition among individuals. In this study, we will use this model of competition to explain the changes in social inequality at Kiwulan, an Iron Age site in northesatern Taiwan, which covers the presence of the Europeans, including the Spanish and the Dutch, in the 17th century and the Chinese in the 19th century. 

# Exponential random graph models

<!--current approaches to network analysis-->
Exponential Random Graph Models (ERGMs) are an important family of statistical models for networks that allows directly modeling for the formation of edges, or ties, between nodes [@Robins2007]. The assumption is that an observed network is randomly generated depending on the presence or absence of other edges. ERGMs differs from traditional network approaches that edges are only formed according to their corresponding predictors separately without taken into account the interactions among edges [@Morris2008]. In statistical models, networks are viewed as dependent variables, the formation of a tie is also influenced by network dependencies, and the attributes of nodes/edges [@Snijders2011]. For example, nodes with similar attributes is more likely to form a relationship, such as friendship between people with the same hobby. Similarly, for network dependencies, reciprocity represents the likelihood between two actor's to be tied because of shared properties, and transitivity means two actors tend to be linked via a relationship with a third actor in common. We can model those network statistics as direct functions of ties, where the probability of occurrence of ties can be specified and hypothesized [@Morris2008; @Robins2007]. Network specification under assumptions and contexts is useful for exploring archaeological network. For instance, @Amati2019 demonstrated how to reconstruct of three different networks consisting of 15 sites (AD 100 to 400) in the Caribbean to explore their interaction mechanisms using ERGMs. Three network models test different hypotheses for the relationship between sites based on proximity, inter-cultural, and exchange network to explain the presence of hub sites they observed. They found that the presence of hubs can be efficiently explained by multiple interdependent mechanisms instead of only one model exclusively. However, they also pointed out some limitations of ERGMs, such as static outcome and sensitivity to missing data. In addition, the complex computational processes, such as intractable normalizing constant and degeneracy due to extreme graphs, limits the widely use of ERGMs. Here, we used ERGMs in a Bayesian framework that allows for parameter inference using Markov chain Monte Carlo (MCMC) algorithm which avoid the need for computationally intensive calculations of the normalizing constants [@Caimo2011]. MCMC algorithms can produce approximate maximum likelihood estimators and graph statistics for further specification of transitivity and heterogeneity, this enables ERGMs to deal with complicated dependence patterns with a ease [@Snijders2006].
